        ...
        public function importCustomer($data) {
            ...
            if ($query->num_rows > 0) {
                ...
                $this->db->query("UPDATE " . DB_PREFIX . "customer SET 
                    ...
                    newsletter = '" . $this->db->escape($data['newsletter']?? '') . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : json_encode([])) . "',
                ...
                $this->db->query("INSERT INTO " . DB_PREFIX . "customer SET 
                    ...
                    ip = '" . $this->db->escape($data['ip']?? '') . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : json_encode([])) . "',
                    status = '" . (int)$data['status'] . "',
                    safe = '" . (int)$data['safe'] . "', 
                    date_added = '" . $this->db->escape($data['date_added']?? '') . "'");
    ...
    if(!empty($affiliateDataArray['payment'])){
       ...
       if(!empty($affiliateDataArray['payment'])){
           ...
           $payment ='cheque';
            ...
            status = '" . (int)$affiliateDataArray['status'] . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : json_encode([])) . "',
        ...
        if(!empty($affiliateDataArray['date_added'])){
             ...
             $this->db->query("INSERT INTO `" . DB_PREFIX . "customer_affiliate` SET `customer_id` = '" . (int)$customer_id . "', `company` = '" . $this->db->escape((string)$affiliateDataArray['company']) . "', `website` = '" . $this->db->escape((string)$affiliateDataArray['website']) . "', `tracking` = '" . $this->db->escape((string)$affiliateDataArray['tracking']) . "', `commission` = '" . (float)$affiliateDataArray['commission'] . "', `tax` = '" . $this->db->escape((string)$affiliateDataArray['tax']) . "', `payment_method` = '" . $this->db->escape((string)$payment) . "', `cheque` = '" . $this->db->escape((string)$affiliateDataArray['cheque']) . "', `bank_account_number` = '" . $this->db->escape((string)$affiliateDataArray['bank_account_number']) . "', `custom_field` = '" . $this->db->escape(isset($affiliateDataArray['custom_field']) ? json_encode($affiliateDataArray['custom_field']) : json_encode([])) . "', `status` = '" . (bool)($affiliateDataArray['status'] ?? 0) . "', `date_added` = '". $this->db->escape($date_added)."'");
