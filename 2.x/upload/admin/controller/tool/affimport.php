...
class ControllerToolaffimport extends Controller { 
	...
	public function index() {	
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->user->hasPermission('modify', 'tool/affimport')) {
			...
			if ($content) {
				...
				*/
				...
				////////////////////////// Started Import work  //////////////
				$this->response->redirect($this->url->link('tool/affimport', 'token=' . $this->session->data['token'], 'SSL'));
			} else {
		...
		$data['import'] = $this->url->link('tool/affimport', 'token=' . $this->session->data['token'], 'SSL');

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('tool/affimport.tpl', $data));
	}
